...
//             try {
//                 const response = await axios.get('https://daroomokamel.ir/plugintest/wp-json/Nv-admin/v1/departments');
...
const NewTicketModal = ({ isOpen, onClose, onSubmit }) => {
    ...
    // const apiUrl = 'https://daroomokamel.ir/plugintest/wp-json/Nv-adminTickets/v1/getusers'
    const apiUrl = Home_URL + 'wp-json/Nv-adminTickets/v1/getusers';

    // Fetch departments from the API
    useEffect(() => {
        const fetchDepartments = async () => {
            try {
                const response = await axios.get(Home_URL + 'wp-json/Nv-clientTickets/v1/departments');
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md mx-auto">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    {/* Dropdown Field */}
                    <div>
                        <label className="block text-sm font-medium text-gray-700">انتخاب کاربر:</label>
                        <ComboDropdown apiUrl={apiUrl} onProductSelect={handleProductSelect} />
                    </div>
